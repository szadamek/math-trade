...
def fitness_function(chromosome):
    ...
    total_exchanges = sum(len(cycle) for cycle in chromosome)
...
def reconstruct_exchanges(cycles, item_owner, item_name, user_lower_to_original, metrics):
    ...
    for cycle in cycles:
        ...
        for i in range(n):
            ...
            # Inicjalizacja transakcji dla użytkowników
            user_transactions.setdefault(giver_user, {'items_given': [], 'items_received': []})
            user_transactions.setdefault(receiver_user, {'items_given': [], 'items_received': []})

            user_transactions[giver_user]['items_given'].append(item_given)
            user_transactions[receiver_user]['items_received'].append(item_given)

    # Zliczenie liczby wymian
    metrics["num_exchanges"] = sum(len(v.get('items_given', [])) for v in user_transactions.values())
...
def save_metrics_to_json(all_metrics,
                         output_file='C:\\Users\\szyma\\Desktop\\System wspomagania i monitorowania procesu wymiany gier\\benchmark\\benchmark_metrics_genetic_algorithm_max_exchanges.json'):
...
def process_file(file_path, output_graph_path, metrics, max_cycle_length=6, population_size=100, num_generations=200,
    ...
    metrics_to_save = {
        ...
        "num_cycles_found": metrics.get("num_cycles_found", 0),
        "num_exchanges": metrics.get("num_exchanges", 0),
        "overall_effectiveness_percent": metrics.get("overall_effectiveness_percent", 0.0),
        "participation_percent": metrics.get("participation_percent", 0.0)
    }

    benchmark_file = r"C:\Users\szyma\Desktop\System wspomagania i monitorowania procesu wymiany gier\benchmark\benchmark_metrics_tradegenie.json"
    ...
    existing_data[base_name] = metrics_to_save
