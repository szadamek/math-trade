...
from algorithms.trade_maximizer_max_players import (
    warn,
    load_data,
...
def test_build_exchange_graph(sample_data):
    ...
    # Sprawdzenie konkretnych krawędzi
    assert G.has_edge("item1", "item2")
    assert G.has_edge("item2", "item1")


def test_find_exchange_cycles_ilp(sample_data):
    metrics = {"num_warnings": 0}  # Inicjalizacja num_warnings
    data = load_data(str(sample_data), metrics)
    users = data.get('users', {})
    items = data.get('items', {})
    ...
    # Mockowanie funkcji print, aby uniknąć wyświetlania podczas testów
    with patch("builtins.print"):
        selected_cycles = find_exchange_cycles_ilp(G, item_owner, user_lower_to_original, max_cycle_length=10,
                                                   metrics=metrics)

    # Oczekiwane cykle: [["item1", "item2"]] lub [["item2", "item1"]]
    # W zależności od implementacji ILP
    assert len(selected_cycles) == 1
    assert set(selected_cycles[0]) == {"item1", "item2"}

    # Sprawdzenie, czy cykl jest poprawny
    cycle = selected_cycles[0]
    assert len(cycle) == 2
    assert item_owner[cycle[0]] == "alice"
    assert item_owner[cycle[1]] == "bob"

    # Sprawdzenie miar
    assert metrics["num_cycles_found"] == 1
    assert metrics["ilp_num_variables"] > 0
    assert metrics["ilp_num_constraints"] > 0
    assert metrics["ilp_solving_time_seconds"] > 0
...
def test_process_file(sample_data, tmp_path):
    # Zdefiniuj ścieżkę do pliku wyjściowego grafu
